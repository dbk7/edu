<?php
    ...
    $r = $con->query("SELECT SUM(barang_masuk_details.qty) AS total_qty, SUM(barang_masuk_details.total_harga_beli) AS total_hargabeli, SUM(barang_masuk_details.total_harga_jual) AS total_hargajual FROM barang INNER JOIN (barang_masuk INNER JOIN barang_masuk_details ON barang_masuk.kode_barangmasuk = barang_masuk_details.kode_barangmasuk) ON barang.id_barang = barang_masuk_details.id_barang WHERE barang.id_barang = '$id_barang' AND date(tgl_barangmasuk) between DATE('$tgl_mulai') AND DATE('$tgl_akhir')");
    foreach ($r as $rr) { 
    ...
    '<html>
        ...
        <center><strong><span style="font-size:160%;">Laporan Barang Masuk Berdasarkan Item</span></strong></center>
        <br>
            <table width="50%" border="1">
              <tbody>
                <tr>
                  <th width="25%" scope="row">Item</th>
            ...
            <table width="100%" border="1">
                <tbody>
                    ...
                    $s = $con->query("SELECT barang_masuk.tgl_barangmasuk, barang_masuk_details.kode_barangmasuk, barang_masuk_details.id_barang, barang.nama_barang, barang_masuk_details.qty, barang_masuk_details.harga_beli, barang_masuk_details.harga_jual FROM barang INNER JOIN (barang_masuk INNER JOIN barang_masuk_details ON barang_masuk.kode_barangmasuk = barang_masuk_details.kode_barangmasuk) ON barang.id_barang = barang_masuk_details.id_barang WHERE barang.id_barang = '$id_barang' AND date(tgl_barangmasuk) between DATE('$tgl_mulai') AND DATE('$tgl_akhir') ORDER BY barang_masuk.tgl_barangmasuk ASC");
    ...
    $dompdf->stream('LapBarangMasuk-'.$nama_barang, array('Attachment'=>0));
