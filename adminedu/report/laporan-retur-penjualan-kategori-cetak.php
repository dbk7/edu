<?php
    ...
    $r = $con->query("SELECT kategori.nama_kategori, SUM(retur_penjualan_details.qty_retur) AS total_qty, SUM(retur_penjualan_details.total_retur) AS grandtotal FROM kategori INNER JOIN ((penjualan INNER JOIN retur_penjualan ON penjualan.kode_penjualan = retur_penjualan.kode_penjualan) INNER JOIN (barang INNER JOIN retur_penjualan_details ON barang.id_barang = retur_penjualan_details.id_barang) ON retur_penjualan.kode_returpenjualan = retur_penjualan_details.kode_returpenjualan) ON kategori.id_kategori = barang.id_kategori WHERE barang.id_kategori = '$kategori' AND date(retur_penjualan.tgl_returpenjualan) between DATE('$tgl_mulai') AND DATE('$tgl_akhir')");
    foreach ($r as $rr) { 
    ...
    '<html>
        ...
        <center><strong><span style="font-size:160%;">Laporan Retur Penjualan Berdasarkan Kategori</span></strong></center>
        <br>
    ...
    $dompdf->stream('LapReturPenjualan-'.$kategori, array('Attachment'=>0));
